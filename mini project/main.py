...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        try:
            ...
            print(f"Warning: Symptom '{item}' not found in symptoms_dict. Skipping.")
    try:
        predicted_index = svc.predict([input_vector])[0]
        predicted_disease = diseases_list.get(predicted_index, "Unknown Disease")
    except Exception as e:
        print(f"Error during prediction: {e}")
        predicted_disease = "Unable to predict. Please try again later."
...
def home():
    if request.method == 'POST':
        ...
        # Validate that symptoms are provided and not misspelled
        if not symptoms or symptoms.lower() == "symptoms":
            message = "Please provide valid symptoms. Ensure they are correctly spelled and separated by commas."
            return render_template('index.html', message=message)

        # Process symptoms input
        user_symptoms = [s.strip() for s in symptoms.split(',') if s.strip() in symptoms_dict]

        # Check if at least one symptom was valid
        if not user_symptoms:
            message = "None of the provided symptoms matched known symptoms. Please try again."
            return render_template('index.html', message=message)
        ...
        return render_template(
            ...
            dis_des=dis_des,
            my_precautions=precautions[0],  # Simplified display for easier reading
